...
const badgeVariants = {
  default: "bg-surface-800/60 text-surface-300 border-surface-700/50",
  gold: "bg-gold-500/10 text-gold-400 border-gold-500/20",
  electric: "bg-electric-500/10 text-electric-400 border-electric-500/20",
  neon: "bg-neon-500/10 text-neon-400 border-neon-500/20",
  crimson: "bg-crimson-500/10 text-crimson-400 border-crimson-500/20",
...
export const Badge: React.FC<BadgeProps> = ({
  children, variant = "default", size = "md", dot, pulse, icon, className,
}) => {
  return (
    <span className={cn(
      "inline-flex items-center font-semibold border rounded-full whitespace-nowrap",
      badgeVariants[variant], sizes[size], className
    )}>
      {dot && (
        <span className="relative flex h-1.5 w-1.5">
          {pulse && <span className={cn("animate-ping absolute inline-flex h-full w-full rounded-full opacity-60", dotColors[variant])} />}
